...
func Solve(input_info InputArguments) int {
	return DynamicProgram(input_info.item_map, input_info.backpack_capacity)
}

func DynamicProgram(items map[int]Item, capacity int) int {
	...
	response := values[len(items)][capacity]
	return response
}
